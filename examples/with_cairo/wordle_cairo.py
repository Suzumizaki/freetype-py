...
try:
    ...
    from cairocffi import Context, ImageSurface, FORMAT_A8, FORMAT_ARGB32, Matrix
try:
    # pycairo 1.11+:
...
def run():
    ...
    image_surface = ImageSurface(FORMAT_A8, W, H)
    ctxI = Context(image_surface)
    ...
    for size in sizes:
        ...
        if h < H and w < W:
            ...
            for _, (dx, dy) in zip(range(1000), spiral()):  # avoid infinite loop.
                ...
                if not (x <= w // 2 or y <= h // 2 or x >= (W - w // 2) or y >= (H - h // 2)):
                    ndI = ndarray(shape=(h, w), buffer=image_surface.get_data(), dtype=ubyte, order='C',
                                  offset=(x - w // 2) + image_surface.get_stride() * (y - h // 2),
                                  strides=[image_surface.get_stride(), 1])
                ...
                if checked or (drawn_regions.contains_rectangle(new_region) == REGION_OVERLAP_OUT):
                    ...
                    break
    image_surface.flush()
    image_name = "wordle_cairo.png"
    image_surface.write_to_png(image_name)
    try:
