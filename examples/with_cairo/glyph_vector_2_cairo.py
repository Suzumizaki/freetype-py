...
try:
    ...
    from cairocffi import Context, SVGSurface, Matrix, SurfacePattern, FILTER_BEST
...
def run():
    ...
    surface = SVGSurface('glyph_vector_2_cairo.svg',
                         width_s,
                         height_s)
    ctx = Context(surface)
    ...
    for i in range(len(outline.contours)):
        ...
        for segment in segments:
            if len(segment) == 2:
                ...
                codes.append(CURVE3)
                for ii in range(1, len(segment) - 2):
                    A, B = segment[ii], segment[ii + 1]
                    C = ((A[0] + B[0]) / 2.0, (A[1] + B[1]) / 2.0)
                    verts.extend([C, B])
                    codes.extend([CURVE3, CURVE3])
                verts.append(segment[-1])
                codes.append(CURVE3)
            [tags.pop() for _ in range(len(segment) - 1)]
        VERTS.extend(verts)
        CODES.extend(codes)
        start = end + 1
    ...
    surface.flush()
    image_name = "glyph_vector_2_cairo.png"
